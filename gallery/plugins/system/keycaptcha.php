<?php
/**
* KeyCAPTCHA Plugin for Joomla 1.5.x
* @version $Id: keycaptcha.php 2011-09-26 $
* @package: KeyCAPTCHA
* ===================================================
* @author
...
if ( !class_exists('KeyCAPTCHA_CLASS') ) {
	class KeyCAPTCHA_CLASS
	{
		...
		function KeyCAPTCHA_CLASS($a_private_key='', $a_js_code='')
		{
			if ( $a_private_key != '' )
			{
			...
			$this->p_kc_session_id = uniqid() . '-4.1.3.12';
	...
	function getParamsForForm( $aextname, $atask, $aview, $forcheck=false, $content='' ) {
		...
		$viewmap = array( 
			...
			"com_virtuemart" => array(
				"shop.registration,,KC_VirtueMartRegistration" => "",
				"checkout.index::adminForm,,KC_RegistrationForm" => "",
				"shop.ask::emailForm|send-#-fs,,KC_VirtueMartAsk" => "",
				...
				"kc_find_form_by_fields_3,KC_VirtueMartAsk" => array (
					"email", "name", "text"
				),
			),
			// for contact us
			...
			"com_comprofiler" => array(
				...
				"kc_find_form_by_fields_2,KC_RegistrationForm" => array (
					"checkemail",
				),
				"kc_find_form_by_fields_3,KC_RegistrationForm" => array (
					"checkusername",
				),
			),
			// JComments
		...
		$kc_o = new KeyCAPTCHA_CLASS($this->params->get('keycaptcha_site_private_key'), str_replace( '#BUTTONS#', implode( ',', $all_submit_names ), $kc_code ) );
